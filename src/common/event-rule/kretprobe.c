...
enum lttng_error_code lttng_event_rule_kretprobe_populate(struct lttng_event_rule *rule, uid_t uid, gid_t gid)
{
	...
	status = lttng_event_rule_kretprobe_get_filter(rule, &filter);
	...
	kretprobe->internal_filter.filter = strdup(filter);
	if (kretprobe->internal_filter.filter == NULL) {
		ret_code = LTTNG_ERR_NOMEM;
		goto end;
	}

	ret = run_as_generate_filter_bytecode(kretprobe->internal_filter.filter, uid, gid, &bytecode);
	if (ret) {
		ret_code = LTTNG_ERR_FILTER_INVAL;
	}

	kretprobe->internal_filter.bytecode = bytecode;
	bytecode = NULL;
