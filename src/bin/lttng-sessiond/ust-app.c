...
/*
 * Delayed reclaim of a ust_app_token_event_rule object. This MUST be called
 * through a call_rcu().
 */
static void free_ust_app_token_event_rule_rcu(struct rcu_head *head)
{
	struct ust_app_token_event_rule *obj =
		caa_container_of(head, struct ust_app_token_event_rule, rcu_head);
		...
		struct ust_app *app)
{
	...
	call_rcu(&ua_token->rcu_head, free_ust_app_token_event_rule_rcu);
}
 ...
 */
static
int set_ust_filter(struct ust_app *app, const struct lttng_filter_bytecode *bytecode, struct lttng_ust_object_data *ust_object)
{
	int ret;
...
static
void init_ust_event_notifier_from_event_rule(
		...
		struct lttng_ust_event_notifier *event_notifier)
{
	...
	strncpy(event_notifier->event.name, pattern, LTTNG_UST_SYM_NAME_LEN - 1);
 ...
 */
static
int create_ust_token_event_rule(struct ust_app *app,
		struct ust_app_token_event_rule *ua_token)
{
	int ret = 0;
	struct lttng_ust_event_notifier event_notifier;
	...
	init_ust_event_notifier_from_event_rule(ua_token->event_rule,
			&event_notifier);
	...
	if (ret < 0) {
		if (ret != -EPIPE && ret != -LTTNG_UST_ERR_EXITING) {
			abort();
			ERR("Error ustctl create event notifier %s for app pid: %d with ret %d",
					event_notifier.event.name, app->pid, ret);
		} else {
			/*
			 * This is normal behavior, an application can die during the
			 * creation process. Don't report an error so the execution can
			 ...
			 */
			ret = 0;
			DBG3("UST app create event notifier failed. Application is dead.");
		}
		goto error;
	}
	...
	DBG2("UST app event notifier %s created successfully for pid:%d object: %p",
			event_notifier.event.name, app->pid, ua_token->obj);
...
void ust_app_synchronize_tokens(struct ust_app *app)
{
	...
	cds_lfht_for_each_entry (app->tokens_ht->ht, &app_trigger_iter.iter,
		...
		for (unsigned int i = 0; i < count; i++) {
			...
			trigger = lttng_triggers_borrow_mutable_at_index(
					triggers, i);
			assert(trigger);
