		...
		const struct lttng_map *map)
{
	...
	case LTTNG_MAP_BITNESS_32BITS:
		kernel_map->counter_conf.bitness = LTTNG_KERNEL_COUNTER_BITNESS_32BITS;
		break;
	case LTTNG_MAP_BITNESS_64BITS:
		kernel_map->counter_conf.bitness = LTTNG_KERNEL_COUNTER_BITNESS_64BITS;
		break;
	default:
		abort();
	}
		...
		struct lttng_kernel_event_notifier *kernel_event_notifier)
{
	 ...
	 * "lttng_event_rule_generate_kernel_event_notifier"
	 */
	...
	case LTTNG_EVENT_RULE_TYPE_KERNEL_PROBE:
	{
		...
		if (symbol_name) {
			strncpy(kernel_event_notifier->event.u.kprobe.symbol_name,
	...
	case LTTNG_EVENT_RULE_TYPE_TRACEPOINT:
		...
		kernel_event_notifier->event.instrumentation = LTTNG_KERNEL_TRACEPOINT;
		(void) lttng_event_rule_tracepoint_get_pattern(rule, &name);
		ret = LTTNG_OK;
		break;
	case LTTNG_EVENT_RULE_TYPE_SYSCALL:
		kernel_event_notifier->event.instrumentation = LTTNG_KERNEL_SYSCALL;
		kernel_event_notifier->event.u.syscall.abi = LTTNG_KERNEL_SYSCALL_ABI_ALL;
		kernel_event_notifier->event.u.syscall.entryexit = LTTNG_KERNEL_SYSCALL_ENTRY;
		kernel_event_notifier->event.u.syscall.match = LTTNG_KERNEL_SYSCALL_MATCH_NAME;
		(void) lttng_event_rule_syscall_get_pattern(rule, &name);
		ret = LTTNG_OK;
		break;
	default:
		ERR("Unknown kernel event rule instrumentation type (%d)",
	...
	/* Copy event name */
	strncpy(kernel_event_notifier->event.name, name, LTTNG_KERNEL_SYM_NAME_LEN);
	kernel_event_notifier->event.name[LTTNG_KERNEL_SYM_NAME_LEN - 1] = '\0';
...
{
	struct ltt_kernel_token_event_rule *rule = caa_container_of(
			rcu_node, struct ltt_kernel_token_event_rule, rcu_node);
