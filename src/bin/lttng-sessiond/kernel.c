		...
		struct lttng_map_key *key)
{
	...
	/* Add event to event list */
	rcu_read_lock();
	lttng_ht_node_init_u64(&event_counter->ht_node, event_counter->action_tracer_token);
	lttng_ht_add_unique_u64(kmap->event_counters_ht, &event_counter->ht_node);
	rcu_read_unlock();
	kmap->event_count++;

	DBG("Event counter %s created (fd: %d)", event_counter->event->name,
			event_counter->fd);
	error_code_ret = LTTNG_OK;
