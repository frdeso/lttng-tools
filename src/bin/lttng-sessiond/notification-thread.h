 ...
 *   8) registration of an ust application,
 *   9) unregistration of an ust application,
 *
 ...
 *
 * 8) Unregistration of an ust application
...
struct notification_thread_state {
	...
	/*
	 * Read side of pipes used to receive tracer events. As their name imply
	 * tracer event source are generated by either registered UST
	 * applications or kernel tracer.
	 *
	 * The list is not protected by a lock since add and remove operations
	 * are currently done only by a single notification thread.
	 */
