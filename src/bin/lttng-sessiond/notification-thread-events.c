	...
	enum lttng_error_code *_cmd_result)
{
	...
	DBG3("[notification-thread] Adding tracer event source fd: %d to pollset", fd);

	/* Adding the read side pipe to the event poll */
	ret = lttng_poll_add(&state->events, fd, LPOLLIN | LPOLLERR);

	if (ret < 0) {
		ERR("[notification-thread] Failed to add tracer event source to pollset");
...
int handle_notification_thread_tracer_event_source_remove(
		struct notification_thread_state *state, int fd)
{
	int ret;
	enum lttng_error_code cmd_result;
		...
		if (trigger_needs_tracer_notifier(trigger)) {
			...
			if (error_accounting_status != EVENT_NOTIFIER_ERROR_ACCOUNTING_STATUS_OK) {
				if (error_accounting_status == EVENT_NOTIFIER_ERROR_ACCOUNTING_STATUS_NO_INDEX_AVAILABLE) {
					DBG("Event notifier group error accounting counter full.");
					*cmd_result = LTTNG_ERR_EVENT_NOTIFIER_ERROR_ACCOUNTING_FULL;
				} else {
					ERR("Error registering event notifier for error accounting");
		...
		struct notification_thread_state *state)
{
	...
	DBG("[notification-thread] Received `%s` command", notification_command_type_str(cmd->type));
	switch (cmd->type) {
	case NOTIFICATION_COMMAND_TYPE_REGISTER_TRIGGER:
		...
		struct lttng_event_notifier_notification *notification)
{
	...
	if (caa_unlikely(!node)) {
		 ...
		 * to situation where the trigger is removed but we still get
