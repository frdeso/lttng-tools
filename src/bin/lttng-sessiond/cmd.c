		...
		enum tracer_executed_action_state state)
{
	...
	case TRACER_EXECUTED_ACTION_STATE_REGISTER:
		ret = map_event_ust_enable_tracepoint(usess,
	...
	case TRACER_EXECUTED_ACTION_STATE_UNREGISTER:
		ret = map_event_ust_disable_tracepoint(usess,
			map, tracer_token, (char *) pattern, key, LTTNG_EVENT_TRACEPOINT,
			LTTNG_EVENT_LOGLEVEL_ALL, 0, NULL, NULL, NULL, false);
		assert(ret == LTTNG_OK);
		break;
		...
		struct lttng_map_content **return_map_content)
{
	...
	if (domain == LTTNG_DOMAIN_KERNEL) {
		...
		if (session->ust_session) {
			...
			if (umap) {
				ret = ust_app_map_list_values(usess, umap, app_bitness,
						return_map_content);
				if (ret) {
		...
		struct lttng_map_content **return_map_content)
{
	...
	ret = list_map_values(domain, session_name, map_name, app_bitness,
			return_map_content);
	if (ret != LTTNG_OK) {
