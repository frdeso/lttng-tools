...
void print_event_rule_tracepoint(const struct lttng_event_rule *event_rule)
{
	...
	if (event_rule_status == LTTNG_EVENT_RULE_STATUS_OK) {
		...
		case LTTNG_LOG_LEVEL_RULE_TYPE_EXACTLY:
			llr_status = lttng_log_level_rule_exactly_get_level(log_level_rule, &log_level);
			log_level_op = "==";
			break;
		case LTTNG_LOG_LEVEL_RULE_TYPE_AT_LEAST_AS_SEVERE_AS:
			log_level_op = "<=";
			llr_status = lttng_log_level_rule_at_least_as_severe_as_get_level(log_level_rule, &log_level);
			break;
		default:
			abort();
		}
...
void print_event_rule_kernel_probe(const struct lttng_event_rule *event_rule)
{
	...
	event_rule_status = lttng_event_rule_kernel_probe_get_location(event_rule, &location);
	if (event_rule_status != LTTNG_EVENT_RULE_STATUS_OK) {
...
void print_event_rule_userspace_probe(const struct lttng_event_rule *event_rule)
{
	...
	event_rule_status = lttng_event_rule_userspace_probe_get_location(event_rule, &location);
	if (event_rule_status != LTTNG_EVENT_RULE_STATUS_OK) {
