...
struct parse_event_rule_res parse_event_rule(int *argc, const char ***argv)
{
	...
	*argv += consumed_args;
	...
	case LTTNG_EVENT_RULE_TYPE_USERSPACE_PROBE:
	{
		...
		event_rule_status = lttng_event_rule_userspace_probe_set_location(res.er, userspace_probe_location);
		if (event_rule_status != LTTNG_EVENT_RULE_STATUS_OK) {
			ERR("Failed to set userspace probe event rule's location.");
			goto error;
		}

		event_rule_status = lttng_event_rule_userspace_probe_set_event_name(res.er, tracepoint_name);
		if (event_rule_status != LTTNG_EVENT_RULE_STATUS_OK) {
...
int cmd_add_trigger(int argc, const char **argv)
{
	...
	if (fire_once_after_str) {
		...
		trigger_status = lttng_trigger_set_firing_policy(trigger,
			LTTNG_TRIGGER_FIRING_POLICY_ONCE_AFTER_N, threshold);
		if (trigger_status != LTTNG_TRIGGER_STATUS_OK) {
	...
	if (user_id) {
		...
		trigger_status = lttng_trigger_set_owner_uid(trigger,
			uid);
		if (trigger_status != LTTNG_TRIGGER_STATUS_OK) {
